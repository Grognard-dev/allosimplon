...
<body>
    ...
    <label class="shadow text-gray-900 border-gray-900 .bg-center focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded m-4 "><b>Nom de l'Acteur</b></label><br>
        ...
        move_uploaded_file($_FILES['photo']['tmp_name'], 'photoacteur/' . $_FILES['photo']['name']);
         if($_FILES['photo']['type'] === 'image/jpeg'){
                        ...
                        $_SESSION['flash'] = " Pas le bon format d'image, format accepter jpeg,png";
                         header('location: insertion_acteur.php');
                        die;
                    }
                    if($image === false){
                        unlink($chemin);
